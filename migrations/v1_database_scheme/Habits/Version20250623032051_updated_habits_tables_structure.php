...
final class Version20250623032051_updated_habits_tables_structure extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        if ($sm->tablesExist('tasks')) {
            ...
            if (array_key_exists('habit_id', $columns) || array_key_exists('pomodoro_count', $columns) || array_key_exists('pomodoro_id', $columns)) {
                $this->addSql('ALTER TABLE tasks ADD COLUMN purpose_id INTEGER DEFAULT NULL');

                if (array_key_exists('habit_id', $columns)) {
                    $this->addSql('ALTER TABLE tasks DROP COLUMN habit_id');
                }

                if (array_key_exists('pomodoro_count', $columns)) {
                    $this->addSql('ALTER TABLE tasks DROP COLUMN pomodoro_count');
                }

                if (array_key_exists('pomodoro_id', $columns)) {
                    $this->addSql('ALTER TABLE tasks DROP COLUMN pomodoro_id');
                }
            }
        }
    }
    ...
    public function down(Schema $schema): void
    {
        ...
        if ($sm->tablesExist('date_daily')) {
            $this->addSql('ALTER TABLE date_daily ADD COLUMN habit_id INTEGER NOT NULL');
        }

        if ($sm->tablesExist('date_weekly')) {
            $this->addSql('ALTER TABLE date_weekly ADD COLUMN habit_id INTEGER NOT NULL');
        }

        if ($sm->tablesExist('tasks')) {
            $this->addSql('ALTER TABLE tasks ADD COLUMN habit_id INTEGER NOT NULL');
            $this->addSql('ALTER TABLE tasks ADD COLUMN pomodoro_id INTEGER DEFAULT NULL');
            $this->addSql('ALTER TABLE tasks ADD COLUMN pomodoro_count INTEGER DEFAULT NULL');
            $this->addSql('ALTER TABLE tasks DROP COLUMN purpose_id');
        }

        if ($sm->tablesExist('purposes')) {
            $this->addSql('ALTER TABLE purposes ADD COLUMN target_type VARCHAR(255) NOT NULL, ADD COLUMN target_id INTEGER NOT NULL');
        }

        if ($sm->tablesExist('date_repeat_per_month')) {
            $this->addSql('ALTER TABLE date_repeat_per_month ADD COLUMN habit_id INTEGER NOT NULL');
        }

        if ($sm->tablesExist('habits')) {
            $this->addSql('ALTER TABLE habits ADD COLUMN date_type VARCHAR(255) NOT NULL');
        }

        if ($sm->tablesExist('pomodoro_history')) {
            $this->addSql('ALTER TABLE pomodoro_history ADD COLUMN target_type VARCHAR(255) NOT NULL, ADD COLUMN target_id INTEGER DEFAULT NULL');
        }
    }
}
